...
export default {
    ...
    async editUsuario({ commit },objeto) {
        try {
            commit('setLoading', true )
            const {data} = await api.put(`/usuarios/${objeto.id}`,objeto)
            if ( !data ){
                return
            }
            const dataFormateada = {
                ...
                estado: data.estado,
                id: data.id,
                rol: data.rol == 'ADMIN_ROLE' ? 'Administrador' : 'Usuario',
                email: data.email,
            };
            commit('editUsuario', dataFormateada )
            return data;
        } catch (ex) {
            commit('setError', ex.response.data.errors )
        }finally{
